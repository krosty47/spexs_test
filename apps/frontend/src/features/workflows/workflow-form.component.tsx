        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-3">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (selectedChannel === 'ALL') {
                    const hasInApp = currentRecipients.some(
                      (r) => r.channel === 'IN_APP' && r.destination === selectedUser.id,
                    );
                    const hasEmail = currentRecipients.some(
                      (r) => r.channel === 'EMAIL' && r.destination === selectedUser.email,
                    );
                    if (!hasInApp) {
                      recipientsField.append({ channel: 'IN_APP', destination: selectedUser.id });
                    }
                    if (!hasEmail && emailEnabled) {
                      recipientsField.append({
                        channel: 'EMAIL',
                        destination: selectedUser.email,
                      });
                    }
                  } else if (selectedChannel === 'IN_APP') {
                    const exists = currentRecipients.some(
                      (r) => r.channel === 'IN_APP' && r.destination === selectedUser.id,
                    );
                    if (!exists) {
                      recipientsField.append({ channel: 'IN_APP', destination: selectedUser.id });
            ...
            {recipientsField.fields.map((field, index) => {
              ...
              const matchedUser = userById.get(destination);
              ...
              return (
                <div key={field.id} className="flex items-center gap-3 rounded-md border p-3">
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8 shrink-0 text-[var(--destructive)] hover:bg-[var(--destructive)]/10 hover:text-[var(--destructive)]"
